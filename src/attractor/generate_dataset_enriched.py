"""
generate_dataset_enriched.py
────────────────────────────
Genera el dataset completo de imágenes Fase 2 (enriquecidas) para
Atractores de Lorenz a partir del PKL compartido umap_params.pkl.

Ubicación: src/attractor/generate_dataset_enriched.py

Capas estéticas aplicadas (derivadas de la trayectoria):
  1. Color:    z(t) → colormap completo de la clase
  2. Grosor:   ‖v(t)‖ → linewidth ∈ [0.1, 1.5]
  3. Opacidad: ‖v(t)‖ → alpha ∈ [0.3, 0.7]

Salidas:
  data/images/attractor_lorenz_enriched/
  ├── train/   (908 imágenes)
  ├── val/     (114 imágenes)
  └── test/    (114 imágenes)

  data/shared/
  ├── metadata_attractor_enriched_train.csv
  ├── metadata_attractor_enriched_val.csv
  └── metadata_attractor_enriched_test.csv

Uso:
  cd proyecto-transformacion-texto-imagen
  python src/attractor/generate_dataset_enriched.py
"""
...
OUTPUT_DIR    = os.path.join(PROJECT_ROOT, 'data', 'images', 'attractor_lorenz_enriched')
METADATA_DIR  = os.path.join(PROJECT_ROOT, 'data', 'shared')

# ── Importar módulos del pipeline ────────────────────────────
sys.path.insert(0, BASE_DIR)
from lorenz import map_umap_to_lorenz, solve_attractor
from render import render_attractor_enriched, compute_velocity
...
def generate_split_enriched(params_norm, labels, split_name, output_base):
    """
    Genera imágenes Fase 2 y metadata para un split completo.
    ...
    for i in range(n_samples):
        ...
        try:
            ...
            # 3. Calcular velocidad para metadata
            speed = compute_velocity(trajectory)

            # 4. Renderizar imagen enriquecida
            label = int(labels[i])
            ...
            metadata.append({
                ...
                'speed_min':    round(float(speed.min()), 4),
...
def main():
    print("=" * 60)
    print("  Generador de Dataset — Atractores de Lorenz (Fase 2)")
    print("  Enriquecimiento visual: color(z), grosor(v), opacidad(v)")
    print("=" * 60)
    ...
    for split_name, (params_norm, labels) in splits.items():
        metadata = generate_split_enriched(
        ...
        csv_path = os.path.join(
            METADATA_DIR, f"metadata_attractor_enriched_{split_name}.csv"
        )
    ...
    print(f"\n{'=' * 60}")
    print(f"  RESUMEN FINAL — FASE 2 (ENRIQUECIDA)")
    print(f"{'=' * 60}")
    ...
    print(f"  Metadata: {METADATA_DIR}/metadata_attractor_enriched_*.csv")

    if total_generated == total:
        print(f"\n  ✓ Dataset Fase 2 completo.")
    else:
